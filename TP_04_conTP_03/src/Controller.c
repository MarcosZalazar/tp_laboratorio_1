...
{
	FILE* pArchivoId;
	const int lastID=1001;
	struct stat buffer;

	int existeArchivo=stat("RegistroIdDisponible.txt",&buffer);
...
void controller_createBinaryFile(void)
{
	...
	int existeArchivoBinario=stat("data.bin",&buffer);

	if(existeArchivoBinario==0)
	{
		pArchivoBinario = fopen("data.bin","r+b");
